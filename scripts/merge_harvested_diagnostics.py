...
from src.engine.ai.harvest_validator import clean_and_sanitize, validate_dtc_record, validate_spn_record  # noqa: E402
...
def load_verified() -> tuple[list[dict], list[dict]]:
    ...
    for p in VERIFIED:
        ...
        if isinstance(d, list):
            for i in d:
                (dtcs if "code" in i else spns).append(i)
...
def merge_dtcs(db: dict, items: list[dict]) -> int:
    ...
    for r in items:
        ...
        if not rep.is_valid or not rep.sanitized:
            continue
        s = rep.sanitized
        c = s["code"]
        if c not in db:
    ...
    for r in items:
        ...
        if s.get("fmi") is not None:
            ...
            if fk not in fm:
                ...
                cur = fm[fk]
                upd = False
                if not cur.get("causes") and s.get("causes"):
                    cur["causes"] = s["causes"]
                    upd = True
                if not cur.get("diagnostic_action") and s.get("actions"):
                    cur["diagnostic_action"] = s["actions"]
                    upd = True
                if upd:
                    cur["by"] = "marshal_gatekeeper"
                    cnt += 1
    return cnt
...
def run_merge() -> dict[str, int]:
    ...
    with open(DTC_DB, "r", encoding="utf-8") as f:
        dtc_db = json.load(f)
    with open(SPN_DB, "r", encoding="utf-8") as f:
        spn_db = json.load(f)
